...
def _load() -> LanguageGrammar:
    ...
    Read, normalize, and validate packaged CMake grammar metadata, then build a
    LanguageGrammar.
    """
    ...
    # 不应受到 checkout 配置影响。
    # EN: Text mode normalizes CRLF/CR to LF before hashing. Git may materialize
    # EN: text files with platform-native line endings, but the grammar identity is
    # EN: defined by canonical UTF-8 JSON content rather than checkout policy.
