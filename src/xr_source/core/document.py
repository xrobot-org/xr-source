...
# ---------------------------------------------------------------------------
# 语言无关的文档外观
# EN: Language-neutral document facade
# ---------------------------------------------------------------------------
...
class SyntaxDocument:
    ...
    # EN: High-level edits deliberately return a fresh reparsed snapshot. The
    # EN: low-level tree API can preserve green sharing, but document consumers must
    # EN: never observe stale parser field labels or diagnostics after an edit.
    def replace(
        self: DocumentT,
    ...
    def _reparse(self: DocumentT, source: bytes) -> DocumentT:
        ...
        # EN: Reparse at the language-document boundary so fields, diagnostics, and recovery
        # EN: structure always come from the parser that owns them. Incremental reparsing can
        # EN: be added later without changing the public API.
