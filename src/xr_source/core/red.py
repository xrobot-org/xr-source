...
# ---------------------------------------------------------------------------
# 绑定到具体快照的 red 视图
# EN: Snapshot-specific red views
# ---------------------------------------------------------------------------
...
class SyntaxElement:
    ...
    Snapshot-specific view that adds parent, field, index and byte offset to a green
    element.
    ...
    def __init__(
        ...
        *,
        ...
        Bind a green element to its SyntaxTree and record parent, index, offset, and field
        metadata.
        """
        self._tree = tree
        self._green = green
    ...
    def path(self) -> tuple[int, ...]:
        ...
        # EN: A path is snapshot-local and records structural child indices.
        # EN: It is not a stable object identity that survives document edits.
...
# EN: Red nodes derive parent/offset information from one owning SyntaxTree.
class SyntaxNode(SyntaxElement):
    ...
    def children(self) -> tuple[SyntaxElement, ...]:
        ...
        # EN: Absolute positions are derived here from immutable child widths rather
        # EN: than stored in green nodes. This is what keeps green subtrees reusable.
        offset = self._offset
...
# EN: Central green -> red adapter. Keep construction here so all red views use the
# EN: same parent/index/offset rules.
def _wrap(
    tree: SyntaxTree,
    child: GreenChild,
    *,
    ...
    Create the matching red view for a GreenChild while propagating parent and offset
    information.
    """
    green = child.element
