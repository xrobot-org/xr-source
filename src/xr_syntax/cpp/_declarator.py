...
from xr_syntax.core import GreenChild, GreenElement, GreenNode, GreenToken
...
class _DeclaratorMixin(_ParserSupport):
    ...
    def _parameter_list_looks_declarative(self, open_paren: int, close_paren: int) -> bool:
        ...
        for start, end in parts:
            ...
            if len(significant) == 1 and self.lexemes[significant[0]].kind == "identifier":
                ...
                # Both forms are possible; at top level, follow the C++ most-vexing-parse bias
                # toward a function declaration.
                continue
        return True
    ...
    def _looks_like_declaration(self, start: int, end: int, *, context: str) -> bool:
        ...
        # the source as a declaration. Pointer/reference punctuators remain declarator syntax.
